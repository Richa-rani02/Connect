   ...
   const navigate=useNavigate();
    ...
    return(
        <header className="header flex flex-align-center flex-justify-between w-100 py-0-75 px-2">
         ...
         <Avatar sx={{ height: '55px', width: '55px', backgroundColor:'#818cf8',border: '2px solid #a5b4fc' }} src={user?.profileImg || user?.firstName?.charAt(0)} alt={user?.firstName} onClick={() => navigate(`/profile/${user?.id}`)} />
        </header>
    )
}